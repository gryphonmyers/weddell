...
class RenderResult {
    ...
    async render() {
        ...
        var transformExpression = exp =>
            (exp instanceof SlotMarker || exp instanceof RenderResult || exp instanceof ComponentRenderHandle)
                ? exp.render()
                : Array.isArray(exp)
...
export const createComponentClass = ({ComponentRenderHandle=ComponentNodeRenderHandle, EventEmitter, Error, Store, renderInterval=RENDER_INTERVAL}) => class Component extends EventEmitter {
    ...
    async [RENDER]() {
        ...
        const result = this.template({
            slot: (slotName='default', defaultContent=null) => {
                if (slotName instanceof RenderResult) {
                    defaultContent = slotName;
                    slotName = 'default'
                }
                return new SlotMarker({ slotName, defaultContent, parent: this })
            },
        ...
        // await this[RENDER_SLOTS]()

        // if (this[IS_DIRTY]) {
        //     return this[RENDER]();
        // }
        return this[RENDER_RESULT] = await (this[RENDER_PROMISE] = result.render());
    }
    ...
    constructor({parent=null, id, attrs={}, content=null}={}) {
        ...
        Object.defineProperties(this, {
            ...
            [IS_DIRTY]: {
                set: (val) => {
                    if (isDirty !== val && val) {
                        this.trigger('markeddirty', {key, store});
                    }
                    isDirty = val;
                },
                get: () => isDirty
